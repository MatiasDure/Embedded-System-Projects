...
static void LCD_sendData(LCD_PinDef *enable_pin);
...
static inline void set_pin_output(LCD_PinDef *lcd_pin) {
...
static inline void clear_data_bit(LCD_PinDef *data_pin) {
	data_pin->port->BRR = (1U << data_pin->pin_number);
}

static void write_data_bit(LCD_PinDef *data_pin, uint8_t bit) {
...
static void writeNibbles(LCD_TypeDef *lcd, uint8_t *nibbles) {
	LCD_PinDef *data_pins[4] = {&lcd->db7, &lcd->db6, &lcd->db5, &lcd->db4};
